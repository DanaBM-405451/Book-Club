...
import {
  ArrowLeft, BookOpen, Edit2, Trash2, Eye, Star, Tag, FileText, Clock, Calendar,
} from 'lucide-react';
// Eliminamos 'api' para evitar problemas con el Gateway
...
//  Helper para obtener token (Manual, igual que en el Modal)
const getAuthToken = () => {
...
//  URL DIRECTA AL SERVICIO (Bypass Gateway 4000)
const LIBRARY_URL = process.env.NEXT_PUBLIC_LIBRARY_SERVICE_URL || 'http://localhost:3003';
...
export default function BookDetailsPage() {
  ...
  const loadBookDetails = async () => {
    try {
      ...
      //  PETICIÓN DIRECTA AL PUERTO 3003
      const response = await axios.get(`${LIBRARY_URL}/api/library/books/${params.id}`, {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = getAuthToken();
      //  PETICIÓN DIRECTA AL PUERTO 3003
      await axios.delete(`${LIBRARY_URL}/api/library/books/${params.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleRating = async (rating) => {
    try {
      const token = getAuthToken();
      // ✅ PETICIÓN DIRECTA AL PUERTO 3003
      await axios.put(`${LIBRARY_URL}/api/library/books/${params.id}/rating`, 
        { rating },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <div>
        <div className="flex justify-between text-sm font-ui mb-1 text-neutral-700">
          <span className="font-bold">
            {/* Si es EPUB y currentPage es 0, mostramos porcentaje estimado */}
            {userBook.currentPage > 0 
               ? `${userBook.currentPage} / ${userBook.totalPages || '?'} páginas`
               : `${Math.round(userBook.progressPercent || 0)}% leído` 
            }
          </span>
